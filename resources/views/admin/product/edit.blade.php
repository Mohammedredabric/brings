      ...
      <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-content">
            <form class="form-horizontal" novalidate action="{{Route('product.update',$product -> id)}}" enctype="multipart/form-data" method="post">
              @csrf
              @method('PUT')
              <input type="hidden" value="{{$product -> customer_id}}" name="id">
              <div class="card-body">
                <div class="row">
                  <div class="col-sm-12">
                    <div class="form-group">
                      <div class="controls">
                        <input type="text" name="ref" value="{{old('ref', $product -> ref)}}" class="form-control" placeholder="ref Produit" required
                  ...
                  <div class="col-sm-12">
                    <div class="form-group">
                      <div class="controls">
                        <input type="text" name="name" value="{{old('name', $product -> name)}}" class="form-control" placeholder="Name Produit" required
                  ...
                  <div class="col-sm-12">
                    <div class="form-group">
                      <div class="controls">
                        <input type="number" step="0.1" name="price" value="{{old('price',$product -> price)}}" class="form-control" placeholder="Prix Produit" required
                      ...
                      <select class="custom-select" id="customer" name="customer" required>
                        @isset($customers)
                          @foreach($customers as $customer)
                            <option value="{{$customer->id}}" {{ ($customer->id == $product -> customer_id) ? "selected":""}} >{{$customer->fname}} {{$customer->lname}}</option>
                          @endforeach
                        @endisset
                      </select>
                    </div>
